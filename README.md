...
📖PageOracle — это программа написанная на python для чтения книг на основе искусственного интеллекта, которая позволяет пользователям загружать текстовые документы и задавать вопросы об их содержании. Она интеллектуально обрабатывает книги, использует передовые модели искусственного интеллекта для поиска нужной информации, а затем генерирует точные, контекстно-зависимые ответы или цитаты — и все это с помощью интуитивно понятного графического интерфейса.
...
### Как пользоваться PageOracle в 9 шагов
...
cd pageoracle
```
3. Перейдите в папку `dependencies` 
```bash
cd dependencies
```
...
python gui.py
```
6. Зайдите в настройки и выберите:
...
7. Дождитесь создание векторного хранилища (при последующих заходах в приложение ждать не потребуется) 6-10 мин (при использовании онлайн embedding-модели nvidia/llama-nemotron-embed-vl-1b-v2:free)
8. Принудительно выберите режим работы при необходимости:
...
9. Введите ваш запрос в поле ввода и дождитесь ответа от ИИ.
