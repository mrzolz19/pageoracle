...
4. При вопросе запускается гибридный поиск:
   • Семантический (vector similarity/MMR).
   • Лексический (BM25 по словам).
5. Результаты объединяются и переоцениваются reranker-моделью.
6. LangGraph выбирает путь:
   • Ответить напрямую.
   • Искать контекст.
   • Переписать запрос и повторить поиск.
7. LLM формирует итог в одном из режимов:
   • analysis - цитаты + анализ.
   • quote - точные цитаты с источниками.
...
1. Симптом: ModuleNotFoundError при старте.
   ...
   • Решение: установить dependencies/requirements.txt в активной среде Python.
2. Симптом: GUI не открывается или падает сразу.
   • Причина: неверная Python-среда или битые зависимости tkinter/torch.
   • Решение: проверить интерпретатор, переустановить зависимости, запустить снова.

#### 2 Ошибки настроек провайдера/ключей
1. Симптом: [Ошибка модели] или отказ подключения модели.
   • Причина: неверный API ключ или модель недоступна у выбранного провайдера.
   • Решение: проверить ключ, тариф/доступ, имя модели в настройках.
2. Симптом: ошибка Yandex при инициализации.
   1. Причина: отсутствует YC_FOLDER_ID.
   • Решение: заполнить YC_FOLDER_ID в настройках и повторить инициализацию.

#### 3 Ошибки embedding и индекса
1. Симптом: ошибка про mismatch dimension embedding.
   • Причина: старый индекс создан другой embedding-моделью.
   • Решение: разрешить приложению пересоздать индекс (автоматически), либо удалить папку chroma_intro_db вручную.
2. Симптом: [Ошибка эмбеддингов] или пустые векторы.
   • Причина: недоступен embedding API или неверный embedding ключ.
   • Решение: проверить сеть, ключ embedding, выбранную модель.
3. Симптом: OpenRouter embeddings вернул не JSON или HTTP-ошибку.
   • Причина: временная проблема маршрута/лимита/модели.
   • Решение: повторить позже, сменить embedding-модель, проверить OPENROUTER_API_KEY.

#### 4 Ошибки поиска и качества ответа
1. Симптом: Контекст пуст: ретривер не нашёл фрагментов.
   • Причина: запрос слишком общий/редкий, или книга не загружена.
   • Решение: переформулировать запрос, указать персонажа/главу, проверить список загруженных книг.
2. Симптом: ответы неточные или не по книге.
   • Причина: слабый retrieval_query или шумный контекст.
   • Решение: использовать режим Поиск цитат, сузить запрос, повысить конкретику формулировки.

#### 5 Ошибки истории и файлов
1. Симптом: история не восстанавливается после перезапуска.
   • Причина: повреждён chat_history.json.
   • Решение: удалить/исправить chat_history.json; приложение создаст новый файл.
2. Симптом: книга не добавляется.
   • Причина: файл не найден, нет доступа, уже проиндексирован.
   • Решение: проверить путь, права, логи; при дубликате загрузка корректно пропускается.

#### 6 Сетевые и лимитные ошибки
1. Симптом: URLError/timeout/HTTP 429.
   • Причина: нестабильная сеть или rate limit у провайдера.
   • Решение: подождать, повторить запрос, снизить частоту операций, проверить тариф/квоты.
